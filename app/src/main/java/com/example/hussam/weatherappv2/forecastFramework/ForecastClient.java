 ...
 */
package com.example.hussam.weatherappv2.forecastFramework;

import android.support.annotation.NonNull;
import android.support.annotation.Nullable;
import android.text.TextUtils;
import com.example.hussam.weatherappv2.forecastFramework.*;
...
import com.example.hussam.weatherappv2.forecastFramework.models.Language;
...
public final class ForecastClient
{
    ...
    {
        Retrofit retrofit = new Retrofit.Builder().baseUrl(com.example.hussam.weatherappv2.forecastFramework.Constants.API_BASE_URL)
        ...
        CacheControl cacheControl = new CacheControl.Builder().maxAge(forecastConfiguration.getCacheMaxAge(), TimeUnit.SECONDS).build();
     ...
     *                     {@link com.example.hussam.weatherappv2.forecastFramework.Constants#OPTIONS_EXCLUDE_CURRENLY},
     *                     {@link com.example.hussam.weatherappv2.forecastFramework.Constants#OPTIONS_EXCLUDE_MINUTELY},
     *                     {@link com.example.hussam.weatherappv2.forecastFramework.Constants#OPTIONS_EXCLUDE_HOURLY}
     *                     {@link com.example.hussam.weatherappv2.forecastFramework.Constants#OPTIONS_EXCLUDE_DAILY},
     *                     {@link com.example.hussam.weatherappv2.forecastFramework.Constants#OPTIONS_EXCLUDE_ALERTS},
     *                     {@link com.example.hussam.weatherappv2.forecastFramework.Constants#OPTIONS_EXCLUDE_FLAGS}
    ...
    public Call<Forecast> getForecast(double latitude, double longitude, @Nullable Integer time,
        ...
        Call<Forecast> forecastCall = mService.getForecast(mApiKey, getLocation(longitude, latitude, time), getQueryMapParameters(language, unit, excludeList, extendHourly), mCacheControl);
    ...
    private Map<String, String> getQueryMapParameters(@Nullable Language language, @Nullable Unit unit,
        ...
        if (language != null) {
            queryMap.put(com.example.hussam.weatherappv2.forecastFramework.Constants.OPTIONS_LANGUAGE, language.getText());
        } else if (mLanguage != null) {
            queryMap.put(com.example.hussam.weatherappv2.forecastFramework.Constants.OPTIONS_LANGUAGE, mLanguage.getText());
        }
        if (unit != null) {
            queryMap.put(com.example.hussam.weatherappv2.forecastFramework.Constants.OPTIONS_UNIT, unit.getText());
        } else if (mUnit != null) {
            queryMap.put(com.example.hussam.weatherappv2.forecastFramework.Constants.OPTIONS_UNIT, mUnit.getText());
        }
        if (excludeBlocks != null && !excludeBlocks.isEmpty()) {
            String exclude = TextUtils.join(",", excludeBlocks);
            queryMap.put(com.example.hussam.weatherappv2.forecastFramework.Constants.OPTIONS_EXCLUDE, exclude);
        } else if (mExcludeBlocks != null && !mExcludeBlocks.isEmpty()) {
            String exclude = TextUtils.join(",", mExcludeBlocks);
            queryMap.put(com.example.hussam.weatherappv2.forecastFramework.Constants.OPTIONS_EXCLUDE, exclude);
        }
        if (extendHourly) {
            queryMap.put(com.example.hussam.weatherappv2.forecastFramework.Constants.OPTIONS_EXTEND, com.example.hussam.weatherappv2.forecastFramework.Constants.OPTIONS_EXTEND_HOURLY);
        }
        return queryMap;
    }
}
